...
static bool32 TryChangeBattleTerrain(u32 battler, u32 statusFlag, u8 *timer)
{
    ...
    {
        gFieldStatuses &= ~(STATUS_FIELD_TERRAIN_PERMANENT | STATUS_FIELD_TERRAIN_ANY );
...
static bool32 IsBattlerGrounded2(u8 battlerId, bool32 considerInverse)
{
    ...
    if ((IS_BATTLER_OF_TYPE(battlerId, TYPE_FLYING) && (!considerInverse || !FlagGet(B_FLAG_INVERSE_BATTLE)))
        || isMultitypeOptimizingDefense & F_DYNAMIC_TYPE_1)
        return FALSE;
    return TRUE;
}
...
static u32 CalcAttackStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    else if (gBattleMoves[move].effect == EFFECT_BODY_PRESS || move == MOVE_BEHEMOTH_BASH)
    {
        atkStat = gBattleMons[battlerAtk].defense;
        atkStage = gBattleMons[battlerAtk].statStages[STAT_DEF];
    }
    else if (gBattleMoves[move].effect == EFFECT_ELECTRO_BALL)
    {
...
static u32 CalcDefenseStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    // hail/snow def boost for ice types
...
static u32 CalcFinalDmg(u32 dmg, u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, u16 typeEffectivenessModifier, bool32 isCrit, bool32 updateFlags)
{
    ...
    // check stab
    teravoltTurboblazed = ((moveType == TYPE_FIRE) && (abilityAtk == ABILITY_TURBOBLAZE))
           || ((moveType == TYPE_ELECTRIC) && (abilityAtk == ABILITY_TERAVOLT));

    if ((move != MOVE_STRUGGLE) && (move != MOVE_NONE))
    {
        if ((IS_BATTLER_OF_TYPE(battlerAtk, moveType)) && ((abilityAtk == ABILITY_ADAPTABILITY) || teravoltTurboblazed))
            MulModifier(&finalModifier, UQ_4_12(2.0));

        else if (IS_BATTLER_OF_TYPE(battlerAtk, moveType) || teravoltTurboblazed )
...
s32 CalculateMoveDamage(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, s32 fixedBasePower, bool32 isCrit, bool32 randomFactor, bool32 updateFlags)
{
    return DoMoveDamageCalc(move, battlerAtk, battlerDef, moveType, fixedBasePower, isCrit, randomFactor,
                            updateFlags, CalcTypeEffectivenessMultiplier(move, moveType, battlerAtk, battlerDef, updateFlags, 0));
}
...
{
    *typeEffectivenessModifier = CalcTypeEffectivenessMultiplier(move, moveType, battlerAtk, battlerDef, FALSE, 0);
